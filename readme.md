...
`build`: Find all pages (files of `type` in `dir`) and build them on startup.  Defaults to `false`.  This avoids waiting for the build the first time you request each page.
